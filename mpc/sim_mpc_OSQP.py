...
def sim_mpc_OSQP(
    ...
    for k in range(n_samples):
        ...
        if k > n_delay:
            if hil_mode ==  True:
                y_meas = (np.round(y_sim[id_to_bpm, k - n_delay] * 1000, 0) * 0.001).astype(dtype)
            else:
                y_meas = y_sim[id_to_bpm, k-n_delay].astype(dtype)

            #Observer - State update
            x0_obs_new = (Ao @ x0_obs_old) + (Bo @ u_sim[id_to_cm, k-1].astype(dtype))
            ...
            osqp_result = result.x[:nu_obs]
            assert result.info.status_val in [1, 2], "OSQP solver did not find an optimal solution."

            if hil_mode == True:
                osqp_result = round(osqp_result * 1000000,0) * 0.000001
            u_sim[id_to_cm, k] = osqp_result.astype(np.float64)
